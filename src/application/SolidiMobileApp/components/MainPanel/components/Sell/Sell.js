...
let Sell = () => {
  ...
  let fetchBestPriceForBaseAssetVolume = async () => {
    ...
    // This also avoids any problem with previous best price requests trying to update the state.
    let market = assetBA + '/' + assetQA;
    let params = {market, side: 'SELL', baseOrQuoteAsset: 'base', baseAssetVolume: volumeBA};
    let output = await appState.fetchBestPriceForASpecificVolume(params);
    if (appState.stateChangeIDHasChanged(stateChangeID)) return;
    if (_.isUndefined(output)) {
      // Happens if the request was aborted.
  ...
  let generateBalanceSection = () => {
    ...
    return (
      <Card style={{ 
        backgroundColor: balanceExceeded ? 
        ...
        <Card.Content>
          <Text variant="bodyMedium" style={{ 
            color: balanceExceeded ? 
              materialTheme.colors.onErrorContainer : 
              materialTheme.colors.onSecondaryContainer 
          }}>
            {balanceExceeded ? "⚠️ " : "💰 "}
            Your balance: {balanceBA} {(balanceBA != '[loading]') && assetBA}
            {balanceExceeded && (
              <Text>{"\n"}You don't have enough {assetBA} for this transaction.</Text>
            )}
          </Text>
        </Card.Content>
      </Card>
  ...
  return (
    <View style={{ 
      ...
      >
        ...
        {/* Error Message */}
        {!!errorMessage && (
          <Card style={{ marginBottom: 16, backgroundColor: materialTheme.colors.errorContainer }}>
            <Card.Content>
